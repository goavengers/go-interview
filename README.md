...
5. [Вопросы про кеширование и базам данных](docs/cache_and_db)
    ...
    - Что такое дедлоки (deadlock), почему возникают, как можно не допускать?
    ...
    - Как правильно выбрать тип данных в Mysql, когда нужны:
      ...
      - Целые числа (TINYINT, SMALLINT, INT, BIGINT) и UNSIGNED, длина числовых типов?
      - Большие числа: decimal, что это и как работает?
      - Float | Double VS Decimal, в чем разница, что и как использовать?
...
Что касается Java, то тут больше схожести, чем с Python. Java является таким же компилируемым, строго типизированным языком программирования с возможностью работать в многопоточном режиме.
...
Также в Java для реализации параллелизма используются потоки (threads) и задачи (tasks), и более абстрагированные concurrency API - исполнители (executors), callable и фьючерсы  (future). 
...
В Go с nil слайсом можно совершать те же операции, которые могли бы делать с инициализированным слайсом
...
Самый надежный способ проверить слайс на пустоту в большинстве случаем - это проверить его длину на ноль. Не стоит проверять слайс на nil
...
// окей, теперь попробуем присвоить слайс другому слайсу
// помним то, что слайс является структурой из трех элементов len, cap и указателем н первый элемент массива
// поэтому в sliceCopy мы получаем скопированные значение len и cap, а также указатель на тот же массив, что и у переменной slice
sliceCopy := slice

// пробуем изменить первый элемент в новом слайсе
sliceCopy[1] = 10
...
причина данного поведения в том, что у обоих слайсов один базовый массив, а также у под слайса своя "копия" слайса с полями len и cap и когда мы пытаемся добавить в дочерний слайс элемент, при условии, что в родительском хватает емкости, мы просто перезаписываем значение в базовом массива.
...
1. Какие бывают типы в Go? Целочисленные, дробные, комплексные, структуры, интерфейсы, время и дополнить.
...
1. Что такое runtime (планировщик scheduler)? Как он устроен в Go?
2. Что такое Goroutine (горутина)?
...
6. Что такое GC (garbage collector/сборщик мусора)?
