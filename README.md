...
<details>
  <summary>Ответ</summary>
  <br />
	**Map** — это встроенная хеш-таблица, которая хранит пары ключ-значение. В Go мапа - это ссылочный тип (как слайс или указатель).

	Структура hmap (упрощенно):
```
type hmap struct {
    count     int    // количество элементов (len)
    flags     uint8  // состояние мапы (идет запись/эвакуация и т.д.)
    B         uint8  // логарифм количества bucket'ов (buckets = 2^B)
    noverflow uint16 // количество переполненных bucket'ов (приблизительно)
    hash0     uint32 // случайное число для хеширования (защита от hash-flood)

    buckets    unsafe.Pointer // указатель на массив bucket'ов (размер 2^B)
    oldbuckets unsafe.Pointer // указатель на старые bucket'ы (при эвакуации)
    nevacuate  uintptr        // счетчик прогресса эвакуации

    extra *mapextra // дополнительные данные (для overflow bucket'ов)
}
```
...
```
type bmap struct {
    tophash [8]uint8   // старшие 8 бит хеша для быстрого сравнения
    keys    [8]keytype // ключи (смежные в памяти)
    values  [8]valuetype // значения (смежные в памяти)
    overflow uintptr   // указатель на следующий bucket (при переполнении)
}
```
Интересный факт: В реальности keys и values не объявлены в структуре — они добавляются во время компиляции для оптимизации памяти.
